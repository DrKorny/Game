...
//Adventure start /forest - orc - potato ending /
function start() {
var user = prompt("You are a great potato. You leave the safe village and travel through the lands of South-Earth until you reach a giant forest. You have to decide if you wanna go THROUGH the forest or take a longer route and AVOID the forest. If you don't like adventures you could also walk BACK to the village...but that's lame, right?").toLowerCase();
switch(user) {
    case 'through':
        confirm("You're going through the forest and stumble into a creature which looks like a tree.");
        confirm ("You could either try to talk to him and ask for a way through the forest or you could try to fight this harmless creature and take his stuff.");
        Tree();
        break;
    case 'avoid':
        confirm("You want to take the safe route? No problem but suddenly pinapples! Ähm also you are surrounded by Orcs..");
        confirm("They're holding in position and threaten you to give them all your gold and food.");
        confirm("There are not too many Orcs so you could have a chance but fighting against 20 Orcs might be a bit risky.");
        confirm("But if you can talk properly you could maybe find another solution. So what's next ?");
        Orc();
        break;
    case 'back':
        confirm("You, the Potato of the story and 'great' adventurer, are heading back to the village because your life doesn't need another adventure. Your Story ends here.");
        confirm("END OF THE STORY. But you can restart and explore other endings.");
        break;
    default:
        confirm("Pardon. Please choose if you wanna go THROUGH the forest or want to AVOID it or be like a potato(and go BACK). Try again!");
        }
}
...
function Tree() {
	...
	switch(oneTree) {
		...
		case 'talk':
			...
			break;
		default:
			confirm("You had one job! Choose between ATTACK and TALK. Now try again.");
    }
}
